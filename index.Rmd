...
Here we look at emergency hospital admissions from 2014-2017 for each neighbourhood in Aberdeen. Admission rates are given relative to the average for Scotland and standarised for age and sex differences in the populations.  

Emergency hospital admissions in Aberdeen are **`r 100-(median(simd_aberdeen$emerg)*100)`% lower than the national average**.  

However, there are **large inequalities in emergency admissions across the city**. In different neighbourhoods, the **emergency admissions rate varies from `r simd_aberdeen %>% arrange(emerg) %>% slice_tail() %>% .$emerg*100-100`% HIGHER than the national average to `r 100 -(simd_aberdeen %>% arrange(emerg) %>% slice_head() %>% .$emerg*100)`% LOWER**. 
